...
std::string PlanJsonGenerator::getOperatorType(const OperatorPtr& operatorNode) {
    ...
    if (operatorNode->instanceOf<SourceLogicalOperator>()) {
        if (operatorNode->as<SourceLogicalOperator>()->getSourceDescriptor()->instanceOf<Network::NetworkSourceDescriptor>()) {
            ...
            operatorType = "SOURCE: " + operatorNode->as<SourceLogicalOperator>()->getSourceDescriptor()->getLogicalSourceName();
        }
        ...
        auto joinType = static_cast<int>(operatorNode->as<LogicalJoinOperator>()->getJoinDefinition()->getJoinType());
        if (joinType == 0) {
            operatorType = "INNER_JOIN";
        } else if (joinType == 1) {
            operatorType = "CROSS_JOIN";
        } else {
            operatorType = "JOIN";
        }
