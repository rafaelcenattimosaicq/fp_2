...
WorkerId Topology::registerWorker(WorkerId workerId,
    ...
    if (workerId == INVALID_WORKER_NODE_ID || nodeWithWorkerIdExists(workerId)) {
        ...
        // Remove any stale topology node at the same address:grpcPort before
        // assigning a new ID.  This prevents duplicate nodes from accumulating
        // when a worker crashes and re-registers before the health check
        // detects the failure (race condition).
        std::optional<WorkerId> staleId;
        for (const auto& [otherId, otherNode] : workerIdToTopologyNode) {
            auto lockedOther = otherNode.rlock();
            if ((*lockedOther)->getIpAddress() == address
                && (*lockedOther)->getGrpcPort() == grpcPort) {
                staleId = otherId;
                break;
            }
        }
        if (staleId.has_value()) {
            NES_WARNING("Removing stale topology node {} at {}:{} before re-registration.",
                        staleId.value(), address, grpcPort);
            unregisterWorker(staleId.value());
        }
