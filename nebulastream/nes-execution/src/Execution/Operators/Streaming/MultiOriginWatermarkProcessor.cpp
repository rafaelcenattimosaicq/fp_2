...
uint64_t MultiOriginWatermarkProcessor::updateWatermark(uint64_t ts, SequenceData sequenceData, OriginId origin) {
    ...
    for (size_t originIndex = 0; originIndex < origins.size(); ++originIndex) {
        if (origins[originIndex] == origin) {
            NES_DEBUG("The current watermark for originId {} is {}, update to {}",
                      origins[originIndex],
                      watermarkProcessors[originIndex]->getCurrentValue(),
                      ts)
            // we have to ensure a monotonic increasing ts, thus, we update if ts increases with ts
            if (watermarkProcessors[originIndex]->getCurrentValue() <= ts) {
                watermarkProcessors[originIndex]->emplace(sequenceData, ts);
            } else {// we take the highest seen ts for the originIndex to keep updating the sequenceData
                watermarkProcessors[originIndex]->emplace(sequenceData, watermarkProcessors[originIndex]->getCurrentValue());
            }
            found = true;
        }
    }
