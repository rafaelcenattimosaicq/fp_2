...
struct ChunkStateTest {
    uint64_t lastChunkNumber = INVALID_CHUNK_NUMBER;
...
class MultiOriginWatermarkProcessorTest : public Testing::BaseUnitTest {
  public:
    ...
    void SetUp() override { watermarkBarriers.clear(); }
    ...
    uint64_t emplaceInMockupQueue(const SequenceData& seqDataToInsert, const uint64_t value) {
        ...
        auto nextSeqNumber = INVALID_SEQ_NUMBER;
        auto chunkStateNextSeq = seenSequenceData.find(nextSeqNumber);
        while (chunkStateNextSeq != seenSequenceData.end()) {
            if (chunkStateNextSeq->second.seenChunks - 1 != chunkStateNextSeq->second.lastChunkNumber - INITIAL_CHUNK_NUMBER) {
                break;
            }
 ...
 */
TEST_F(MultiOriginWatermarkProcessorTest, singleThreadSequentialUpdaterTest) {
    ...
    for (auto i = INITIAL_SEQ_NUMBER; i <= updates; i++) {
        watermarkBarriers.emplace_back(
            std::tuple<SequenceData, uint64_t>(/*sequence data*/ {SequenceNumber(i), INITIAL_CHUNK_NUMBER, true}, /*ts*/ i));
    }
 ...
 * As we insert all updates in a sequential fashion we assume that the getCurrentWatermark is equal to the latest processed update.
 */
TEST_F(MultiOriginWatermarkProcessorTest, singleThreadReversSequentialUpdaterTest) {
    auto updates = 10000_u64;
    auto watermarkProcessor = Sequencing::NonBlockingMonotonicSeqQueue<uint64_t>();
    /// preallocate watermarks for each transaction
    for (auto i = INITIAL_SEQ_NUMBER; i <= updates; i++) {
 ...
 * As we insert all updates in a sequential fashion we assume that the getCurrentWatermark is equal to the latest processed update.
 */
TEST_F(MultiOriginWatermarkProcessorTest, singleThreadRandomeUpdaterTest) {
...
TEST_F(MultiOriginWatermarkProcessorTest, concurrentLockFreeWatermarkUpdaterTest) {
    ...
    /// preallocate watermarks for each transaction
    for (auto i = INITIAL_SEQ_NUMBER; i <= updates * threadsCount; i++) {
        watermarkBarriers.emplace_back(
            std::tuple<SequenceData, uint64_t>(/*sequence data*/ {SequenceNumber(i), INITIAL_CHUNK_NUMBER, true}, /*ts*/ i));
    }
...
TEST_F(MultiOriginWatermarkProcessorTest, concurrentUpdatesWithLostUpdateThreadTest) {
 ...
 * is produced
 */
TEST_F(MultiOriginWatermarkProcessorTest, DISABLED_singleThreadedUpdatesWithChunkNumberInRandomFashionTest) {
    auto noSeqNumbers = 10000_u64;
    auto maxChunksPerSeqNumber = 20_u64;
    auto watermarkProcessor = Sequencing::NonBlockingMonotonicSeqQueue<uint64_t>();
    /// preallocate watermarks for each transaction
    for (auto i = INITIAL_SEQ_NUMBER; i <= noSeqNumbers; i++) {
        auto noChunks = 1 + rand() % maxChunksPerSeqNumber;
        for (auto chunk = INITIAL_CHUNK_NUMBER; chunk < INITIAL_CHUNK_NUMBER + noChunks; ++chunk) {
            watermarkBarriers.emplace_back(
        ...
        watermarkBarriers.emplace_back(std::tuple<SequenceData, uint64_t>(
            /*sequence data*/ {SequenceNumber(i), ChunkNumber(noChunks + INITIAL_CHUNK_NUMBER), true},
            /*ts*/ i));
    }
 ...
 */
TEST_F(MultiOriginWatermarkProcessorTest, DISABLED_concurrentUpdatesWithChunkNumberInRandomFashionTest) {
    ...
    /// preallocate watermarks for each transaction
    for (auto i = INITIAL_SEQ_NUMBER; i < noSeqNumbers + INITIAL_SEQ_NUMBER; i++) {
        auto noChunks = 1 + rand() % maxChunksPerSeqNumber;
        for (auto chunk = INITIAL_CHUNK_NUMBER; chunk < noChunks + INITIAL_CHUNK_NUMBER; ++chunk) {
            watermarkBarriers.emplace_back(std::tuple<SequenceData, uint64_t>(/*sequence data*/ {i, chunk, false}, /*ts*/ i));
        }
        watermarkBarriers.emplace_back(std::tuple<SequenceData, uint64_t>(
            /*sequence data*/ {i, ChunkNumber(noChunks + INITIAL_CHUNK_NUMBER), true},
            /*ts*/ i));
    }
...
TEST_F(MultiOriginWatermarkProcessorTest, simpleInsertionsWithSingleChunks) {
    std::vector<BufferMetaDataTest> sequenceData = {
        BufferMetaDataTest{{1, INITIAL_CHUNK_NUMBER, true}, 31_u64},
        BufferMetaDataTest{{2, INITIAL_CHUNK_NUMBER, true}, 63_u64},
        BufferMetaDataTest{{3, INITIAL_CHUNK_NUMBER, true}, 80_u64},
        BufferMetaDataTest{{4, INITIAL_CHUNK_NUMBER, true}, 99_u64},
    };

    auto watermarkProcessor = Sequencing::NonBlockingMonotonicSeqQueue<uint64_t>();

    /// Inserting the first sequence ---> current value should be the timestamp of the first sequence
    watermarkProcessor.emplace(sequenceData[0].sequenceData, sequenceData[0].timestamp);
    EXPECT_EQ(watermarkProcessor.getCurrentValue(), sequenceData[0].timestamp);

    /// Inserting the second sequence ---> current value should be the timestamp of the second sequence
    watermarkProcessor.emplace(sequenceData[1].sequenceData, sequenceData[1].timestamp);
    EXPECT_EQ(watermarkProcessor.getCurrentValue(), sequenceData[1].timestamp);
    ...
    watermarkProcessor.emplace(sequenceData[3].sequenceData, sequenceData[3].timestamp);
    EXPECT_EQ(watermarkProcessor.getCurrentValue(), sequenceData[1].timestamp);
    ...
    watermarkProcessor.emplace(sequenceData[2].sequenceData, sequenceData[2].timestamp);
    EXPECT_EQ(watermarkProcessor.getCurrentValue(), sequenceData[3].timestamp);
}
