...
class NonBlockingMonotonicSeqQueue {
  ...
  public:
    ...
    void emplace(SequenceData sequenceData, T newValue) {
        if (auto opt = chunks.collect(sequenceData, uint64_t(newValue))) {
...
///
    ...
    void emplaceValueInBlock(SequenceNumber seq, T value) {
        ...
        //   NES_DEBUG("sequence number: {} check blocks", seq);
        ...
        if (!sequenceNumberIsNotInPriorBlock) {
            throw Exceptions::RuntimeException("sequence number was in prior block:");
        }
        const auto sequenceNumberIsNotInSubsequentBlock = seq < ((currentBlock->blockIndex * BlockSize) + BlockSize);
        if (!sequenceNumberIsNotInSubsequentBlock) {
            throw Exceptions::RuntimeException("sequence number was in subsequent block:");
            // throw Exceptions::RuntimeException("sequence number: {} was in subsequent block: {}",
            //                                    seq,
