        ...
        try {
            ...
            if (connected) {
                ...
                if (message) {// Check if message was received correctly (not nullptr)
                    ...
                    // Parse the message — skip malformed messages instead of aborting
                    // the entire buffer. A single bad message should not prevent all
                    ...
                    try {
                        if (!inputParser->writeInputTupleToTupleBuffer(receivedMessageString,
                                                                       ...
                                                                       schema,
                                                                       localBufferManager)) {
                            NES_WARNING("MQTTSource::fillBuffer: Skipping unparseable message (parser returned false): '{}'",
                                        receivedMessageString);
                            continue;
                        }
                    } catch (const std::exception& parseError) {
                        NES_WARNING("MQTTSource::fillBuffer: Skipping message due to parse error: {} — message was: '{}'",
                                    parseError.what(), receivedMessageString);
                        continue;
                    }
            ...
            // MQTT transport errors are fatal — cannot continue without a connection.
            NES_ERROR("MQTTSource::fillBuffer: MQTT error: {}", error.what());
            return false;
        } catch (...) {
            ...
            try {
                ...
                NES_ERROR("MQTTSource::fillBuffer: Unexpected error: {}", error.what());
                return false;
            }
        }
