...
bool MQTTSink::writeData(Runtime::TupleBuffer& inputBuffer, Runtime::WorkerContextRef) {
    ...
    // Gracefully handle disconnected state instead of crashing (NES_ASSERT).
    // Attempt reconnection — the broker may have been unreachable at setup()
    // time but is available now.
    if (!connected) {
        NES_WARNING("MQTTSink::writeData: client not connected — attempting reconnect to {}", address);
        lock.unlock();
        connect();
        lock.lock();
        if (!connected) {
            NES_ERROR("MQTTSink::writeData: reconnect failed, dropping buffer");
            return false;
        }
        NES_INFO("MQTTSink::writeData: reconnect succeeded");
    }
...
bool MQTTSink::connect() {
    ...
    // Retry connection up to 3 times with 1-second delays.
    // The MQTT broker may not be immediately reachable (e.g. DNS propagation,
    // broker startup delay, or transient network issues in VPC).
    constexpr int MAX_RETRIES = 3;
    for (int attempt = 1; attempt <= MAX_RETRIES; ++attempt) {
        try {
            // Explicitly request MQTT v3.1.1 (protocol level 4).
            // The Paho C++ MQTTVERSION_DEFAULT may negotiate to v5 which
            // rumqttd rejects when only v4 listeners are configured.
            //
            // keep_alive must be > 0: rumqttd rejects zero keep_alive
            // (Error::ZeroKeepAlive). When messageDelay is 0 the computed
            // interval is 0ns — fall back to 60 seconds.
            auto keepAlive = maxBufferedMessages * minDelayBetweenSends;
            constexpr auto MIN_KEEP_ALIVE = std::chrono::seconds(60);
            if (keepAlive < MIN_KEEP_ALIVE) {
                keepAlive = MIN_KEEP_ALIVE;
            }
            ...
            // Only set username if non-empty; an empty user_name("") would
            // still set the MQTT username flag, which some brokers reject
            // when authentication is not configured.
            if (!user.empty()) {
                builder.user_name(user);
            }
            auto connOpts = builder.finalize();
            NES_DEBUG("MQTTSink::connect: attempt {}/{} to address= {}", attempt, MAX_RETRIES, address);
            client->connect(connOpts);
            connected = true;
            NES_INFO("MQTTSink::connect: connected to {} on attempt {}", address, attempt);
            return true;
        } catch (const mqtt::exception& ex) {
            NES_ERROR("MQTTSink::connect: attempt {}/{} failed: {}", attempt, MAX_RETRIES, ex.what());
    ...
    NES_ERROR("MQTTSink::connect: all {} attempts failed for {}", MAX_RETRIES, address);
    return false;
}
