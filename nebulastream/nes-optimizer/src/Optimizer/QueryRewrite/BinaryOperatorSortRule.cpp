...
QueryPlanPtr BinaryOperatorSortRule::apply(QueryPlanPtr queryPlanPtr) {
    ...
    //Find all join operators in the query plan and sort children individually.
    auto intervalJoinOperators = queryPlanPtr->getOperatorByType<LogicalIntervalJoinOperator>();
    for (const auto& intervalJoinOperator : intervalJoinOperators) {
        sortChildren(intervalJoinOperator);
    }
