...
Query& Seq::window(const Windowing::WindowTypePtr& windowType) const {
    ...
    auto sourceNameLeft = originalQuery.getQueryPlan()->getSourceConsumed();
    auto sourceNameRight = subQueryRhs.getQueryPlan()->getSourceConsumed();
    std::regex r1("-_+");
    ...
    return originalQuery.seqWith(subQueryRhs,
                                 (Attribute(sourceNameLeft) < Attribute(sourceNameRight)),
                                 windowType);//call original seqWith() function
}
