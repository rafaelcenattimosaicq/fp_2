...
class SourceCatalogController : public oatpp::web::server::api::ApiController {
  ...
  public:
    ...
    ENDPOINT("DELETE", "/removePhysicalSource", removePhysicalSource,
             QUERY(String, logicalSourceName, "logicalSourceName"),
             QUERY(String, physicalSourceName, "physicalSourceName"),
             QUERY(UInt64, workerId, "workerId")) {
        NES_DEBUG("SourceCatalogController: removePhysicalSource: REST received request to remove a physical source.");
        try {
            auto removed = requestHandlerService->queueUnregisterPhysicalSourceRequest(
                physicalSourceName, logicalSourceName, WorkerId(workerId));
            if (removed) {
                nlohmann::json success;
                ...
                return errorHandler->handleError(Status::CODE_400,
                    "Unable to remove physical source. Source not found or worker ID mismatch.");
            }
        } catch (const std::exception& exc) {
            NES_ERROR("SourceCatalogController: removePhysicalSource: Exception: {}", exc.what());
            return errorHandler->handleError(Status::CODE_500, exc.what());
        } catch (...) {
            return errorHandler->handleError(Status::CODE_500, "SourceCatalogController: unknown exception.");
        }
    ...
    ENDPOINT("DELETE", "/removeAllPhysicalSourcesByWorker", removeAllPhysicalSourcesByWorker,
             QUERY(UInt64, workerId, "workerId")) {
        NES_DEBUG("SourceCatalogController: removeAllPhysicalSourcesByWorker: REST received request.");
        try {
            auto removed = requestHandlerService->queueUnregisterAllPhysicalSourcesByWorkerRequest(
                WorkerId(workerId));
            nlohmann::json result;
            result["success"] = removed;
            return createResponse(Status::CODE_200, result.dump());
        } catch (const std::exception& exc) {
            NES_ERROR("SourceCatalogController: removeAllPhysicalSourcesByWorker: Exception: {}", exc.what());
            return errorHandler->handleError(Status::CODE_500, exc.what());
        } catch (...) {
            return errorHandler->handleError(Status::CODE_500, "SourceCatalogController: unknown exception.");
        }
