...
TEST_F(QueryDeploymentTest, testIntervalJoinTwoSources) {
    ...
    auto csvSourceType2 = CSVSourceType::create("window2", "test_stream");
    csvSourceType2->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) / "window.csv");
    csvSourceType2->setNumberOfTuplesToProducePerBuffer(7);
    csvSourceType2->setNumberOfBuffersToProduce(4);
    workerConfig2->physicalSourceTypes.add(csvSourceType2);

    auto csvSourceType4 = CSVSourceType::create("window1", "test_stream");
    csvSourceType4->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) / "window4.csv");
    csvSourceType4->setNumberOfTuplesToProducePerBuffer(4);
    csvSourceType4->setNumberOfBuffersToProduce(2);
    workerConfig2->physicalSourceTypes.add(csvSourceType4);
    ...
    std::string outputFilePath = getTestResourceFolder() / "testIntervalJoinsWithDifferentStreamSlidingWindowDistributed.out";
    remove(outputFilePath.c_str());
    ...
    auto query = NES::Query::from("window1")
                     .intervalJoinWith(NES::Query::from("window2"))
                     .where(Attribute("id1") < Attribute("id2"))
                     .lowerBound(EventTime(Attribute("timestamp")), 0, Seconds())
                     .upperBound(3, Seconds())
                     .sink(FileSinkDescriptor::create(outputFilePath, "CSV_FORMAT", "APPEND"));
    ...
    string expectedContent = "window1window2$start:INTEGER(64 bits),window1window2$end:INTEGER(64 "
                             "bits),window1$win1:INTEGER(64 bits),window1$id1:INTEGER(64 bits),window1$"
                             "timestamp:INTEGER(64 bits),window2$win2:INTEGER(64 bits),window2$id2:INTEGER(64 "
                             "bits),window2$timestamp:INTEGER(64 bits)\n"
                             "1300,4300,12,1,1300,1,2,2000\n"
                             "1300,4300,12,1,1300,11,2,2001\n"
                             "1300,4300,12,1,1300,16,2,2002\n"
                             "1300,4300,12,1,1300,1,3,3000\n"
                             "1300,4300,12,1,1300,11,3,3001\n"
                             "1300,4300,12,1,1300,1,3,3003\n"
                             "1300,4300,12,1,1300,1,3,3200\n"
                             "1300,4300,12,1,1300,1,4,4000\n"
                             "12000,15000,1,12,12000,1,13,13000\n"
                             "12000,15000,1,12,12000,1,14,14000\n"
                             "12000,15000,1,12,12000,1,15,15000\n"
                             "13000,16000,1,13,13000,1,14,14000\n"
                             "13000,16000,1,13,13000,1,15,15000\n"
                             "13000,16000,1,13,13000,1,16,16000\n"
                             "14000,17000,1,14,14000,1,15,15000\n"
                             "14000,17000,1,14,14000,1,16,16000\n"
                             "14000,17000,1,14,14000,1,17,17000\n"
                             "15000,18000,1,15,15000,1,16,16000\n"
                             "15000,18000,1,15,15000,1,17,17000\n"
                             "15000,18000,1,15,15000,1,18,18000\n";
